...
const { row, table } = defineProps<TableRowProps<Fertilizer>>()

const isDeleteDialogOpen = ref<boolean>(false)

const handleUpdate = async (id: number, fertilizer: Fertilizer): Promise<void> => {
  await table.options.meta?.update(id, fertilizer)
}

const handleManageLogs = (): void => {
  table.options.meta?.action?.("manage-logs", row.original.id)
}
...
<template>
  <DropdownMenu>
    ...
    <DropdownMenuContent align="end">
      ...
      <DropdownMenuItem @click="handleManageLogs"> Logs </DropdownMenuItem>
      <DropdownMenuSeparator />
