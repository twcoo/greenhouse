...
const handleUpdate = async (id: number, location: PlantingLocation) => {
  await table.options.meta?.update(id, location)
}
...
<template>
  <DropdownMenu>
    ...
    <DropdownMenuContent align="end">
      ...
      <DropdownMenuSeparator />
      <DropdownMenuItem
        @click="table.options.meta?.action?.('manage-status', row.original.id)"
        data-test="manage-status-action"
      >
        Manage Status
      </DropdownMenuItem>
      <DropdownMenuSeparator />
