...
const emit = defineEmits<{
  (e: "submit", payload: PlantingPayload, onError: (err: unknown) => void): void
}>()
...
const selectedVarietyId = ref<string>("")
...
<template>
  <Dialog v-model:open="open">
    <form id="planting-form" @submit.prevent="handleSubmit">
      <DialogContent class="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Add Planting</DialogTitle>
          <DialogDescription>
            Add a new planting record. Select a crop and its variety.
          </DialogDescription>
        </DialogHeader>
        <FieldGroup>
          <Field>
